...
(** val clinear_function : Language3.coq_function -> coq_function option **)

let clinear_function f =
  let enum = enumerate f in
  Some { fn_return = f.Language3.fn_return; fn_params =
  f.Language3.fn_params; fn_temps = f.Language3.fn_temps; fn_code =
  (add_branch f.Language3.fn_entrypoint (clinear_body f enum)) }

(** val clinear_fundef : Language3.coq_function -> coq_function option **)

let clinear_fundef =
  clinear_function

(** val clinear_fundefs :
    Language3.coq_function PTree.t -> coq_function PTree.t option **)
...
(** val clinear_constructor :
    Language3.coq_function option -> coq_function option **)

let clinear_constructor = function
| Some c -> bind (clinear_fundef c) (fun f -> Some f)
| None -> None

(** val clinear_genv : Language3.genv -> genv option **)
...
let clinear_genv ge =
  ...
  bind (clinear_fundefs fundefs) (fun fundefs0 ->
    bind (clinear_methoddefs methoddefs) (fun methoddefs0 ->
      bind (clinear_constructor constructor) (fun constructor0 -> Some
